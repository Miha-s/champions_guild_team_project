...
first_k( Grammar& grammar, int k )
{
    ...
    while ( updated )
    {
        ...
        for ( const SyntaxRule& rule : grammar.get_grammar_rules( ) )
        {
            ...
            firstSets[ rule.get_left_side( ) ] = addition_result;
        }
    }
...
follow_k( Grammar& grammar, int k )
{
    ...
    firstSets[ *grammar.get_non_terminals( ).begin( ) ].insert(
            Terminals { Terminal::EPSILON_TERMINAL } );

    for ( const SyntaxRule& rule : grammar.get_grammar_rules( ) )
    {
        const auto& rightHandSide = rule.get_right_side( );
        for ( int i = 0; i < rule.get_right_side( ).size( ); i++ )
        {
            if ( rule.get_right_side( )[ i ] == rule.get_left_side( ) )
            {
                std::vector< TerminalsSet > result_of_first_k;
                for ( int j = i + 1; j < rule.get_right_side( ).size( ); j++ )
                {
                    if ( rule.get_right_side( )[ j ].is_terminal( ) )
                    {
                        result_of_first_k.push_back( std::unordered_set< Terminals , TerminalsHash >{
                                Terminals { grammar.get_terminal_by_id(
                                        rule.get_right_side( )[ j ].id( ) ) } } );
                    }
                    else
                    {
                ...
                firstSets[ rule.get_left_side() ].insert( kAddition(result_of_first_k));
            }
        }
    }
